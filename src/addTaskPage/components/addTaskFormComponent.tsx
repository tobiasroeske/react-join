...
function AddTaskForm({ state, handleSubmitActions }: AddTaskFormProps) {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (data) => {
    if (user) {
      const newTask = createTaskObject(data, user)
          ...
          <div className="inputContainer">
            ...
            <input
              {...register('title', {
                ...
                maxLength: {
                  value: 25,
                  message: 'Please enter maximal 20 characters'
                }
              })}
              type="text"
              id="title"
        ...
        <div className={styles.actionBtnField}>
          ...
          <button
            ...
            className={classNames(styles.submitBtn, {
              [styles.disabledBtn]: isSubmitting
            })}
            disabled={isSubmitting}
          >
            Create Task{' '}
            <img src="/assets/icons/check_contacts.png" alt="Submit" />
          </button>
        </div>
      </div>
      {showCreatedButton && (
        <div className={styles.createdOverlay}>
          <div className={styles.createdMessage}>Task added to Board</div>
        </div>
      )}
    </form>
  )
